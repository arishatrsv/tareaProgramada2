...
def actualizarDonadorAux(pnombre,ptelefono,pfecha,psangre,ppeso):
    ...
    while validarTelefono(ptelefono)==False:
        ...
        ptelefono=input("Digite el teléfono: ")
    while validarFecha(pfecha)==False:
        print("La fecha debe tener el formato dd/mm/yyyy y ser válida.")
        pfecha=input("Digite la fecha de nacimiento: ")
    while validarTipoSangre(psangre)==False:
        print("El tipo de sangre debe ser O+, O-, A+, A-, B+, B-, AB+ o AB-.")
        psangre=input("Digite el tipo de sangre: ").upper()
    while True:
        try:
            ppeso=float(ppeso)
            if validarPeso(ppeso)==True:
                break
            else:
                print("El peso mínimo para donar es de 50 kg")
                ppeso=input("Digite el peso: ")
        except:
